...
export default function EmpresasCRUD() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setStatusMessage({ type: "error", text: "Error al cargar empresas: " + err.message });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenDelete = (item: Empresa) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!formData.nombre.trim() || !formData.nit.trim()) {
      setStatusMessage({ type: "error", text: "El nombre y el NIT son campos obligatorios." });
      return;
    }
    ...
    try {
      if (modalMode === "create") {
        ...
        if (error) throw error;
        setStatusMessage({ type: "success", text: "Empresa agregada con éxito." });
      } else {
        ...
        const { error } = await supabase
          ...
          .eq("id_empresa", selectedItem.id_empresa);
        if (error) throw error;
        setStatusMessage({ type: "success", text: "Empresa actualizada con éxito." });
      }
      ...
      setStatusMessage({ type: "error", text: "Error al guardar empresa: " + err.message });
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      const { error } = await supabase
        .from("empresa")
        .delete()
        .eq("id_empresa", itemToDelete.id_empresa);

      if (error) throw error;

      setStatusMessage({ type: "success", text: "Empresa eliminada con éxito." });
      ...
      setStatusMessage({ 
        type: "error", 
        text: "Error al eliminar empresa: " + (err.message.includes("violates foreign key constraint") 
          ? "No se puede eliminar porque tiene departamentos o empleados asociados." 
          : err.message)
      });
      setShowDeleteConfirm(false);
      setLoading(false);
      ...
      <div className="md:ml-64 flex flex-col min-h-screen flex-1 min-w-0">
        ...
        <header className="flex justify-between items-center w-full px-section-gap py-4 z-30 bg-surface-container-lowest/80 backdrop-blur-md sticky top-0 border-b border-outline-variant">
          <div className="flex items-center gap-4 flex-1 pl-12 md:pl-0">
            <span className="font-h3 text-h3 font-bold text-primary">Catálogo de Empresas</span>
          </div>
        ...
        <main className="p-4 md:p-8 flex-1 min-w-0">
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h1 className="font-h2 text-h2 text-on-surface tracking-tight">Empresas</h1>
                <p className="text-body-sm text-on-surface-variant">Gestione las entidades y razones sociales de facturación.</p>
              </div>
              <button 
                    ...
                    {loading && data.length === 0 ? (
                      ...
                      data.map((item) => (
                        <tr key={item.id_empresa} className="hover:bg-surface-container/30 transition-colors">
                          ...
                          <td className="px-6 py-4 font-data-tabular">{item.nit}</td>
                          <td className="px-6 py-4 text-on-surface-variant">{item.direccion || "-"}</td>
          ...
          <div className="bg-surface-container-lowest w-full max-w-lg rounded-xl shadow-2xl overflow-hidden border border-outline-variant animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-outline-variant flex justify-between items-center bg-surface-container-low">
              <h3 className="font-h2 text-h2 text-on-surface">
                {modalMode === "create" ? "Agregar Nueva Empresa" : "Editar Empresa"}
              </h3>
              <button
            ...
            <form onSubmit={handleSave}>
              <div className="p-6 space-y-4">
                <div className="space-y-1.5">
                  <label className="block text-label-caps text-on-surface-variant uppercase font-semibold">Nombre de la Empresa *</label>
                  <input 
                ...
                <div className="space-y-1.5">
                  <label className="block text-label-caps text-on-surface-variant uppercase font-semibold">Dirección</label>
                  <input 
                    className="w-full px-4 py-2 bg-background border border-outline-variant rounded-lg focus:ring-1 focus:ring-primary focus:border-primary text-body-base focus:outline-none" 
                    type="text" 
                ...
                <div className="space-y-1.5">
                  <label className="block text-label-caps text-on-surface-variant uppercase font-semibold">Teléfono</label>
                  <input 
                    className="w-full px-4 py-2 bg-background border border-outline-variant rounded-lg focus:ring-1 focus:ring-primary focus:border-primary text-body-base focus:outline-none" 
                    type="text" 
          ...
          <div className="bg-surface-container-lowest w-full max-w-md rounded-xl shadow-2xl overflow-hidden border border-outline-variant animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6">
              <p className="text-body-base text-on-surface">
                ¿Está seguro de que desea eliminar la empresa <strong>{itemToDelete.nombre}</strong>?
              </p>
              <p className="text-body-sm text-on-surface-variant mt-2">
                Esta acción no se puede deshacer. Si la empresa posee departamentos o empleados asociados, la base de datos impedirá la eliminación para proteger la integridad referencial.
              </p>
            </div>
